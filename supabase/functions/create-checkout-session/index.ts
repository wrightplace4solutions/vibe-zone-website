...
serve(async (req) => {
  ...
  try {
    const { packageType, customerEmail, customerName, eventDate, eventDetails }: RequestBody =
      await req.json();
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            unit_amount: selectedPackage.deposit * 100, // Stripe uses cents
          },
          quantity: 1,
        },
      ],
      ...
      success_url: `${origin}/booking?session_id={CHECKOUT_SESSION_ID}&payment_status=success`,
      cancel_url: `${origin}/booking?payment_status=cancelled`,
    });
